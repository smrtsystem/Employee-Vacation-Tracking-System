...
function getTodayMessage(departureStr, returnStr) { 
    ...
    const dep = parseDateLocal(departureStr);
    const ret = parseDateLocal(returnStr); 
    
    if (!dep || !ret) return "⏳ Dates not set"; 
    
    const depMid = new Date(dep.getFullYear(), dep.getMonth(), dep.getDate());
    const retMid = new Date(ret.getFullYear(), ret.getMonth(), ret.getDate());
    const todayMid = today; 
    
    if (todayMid < depMid) { 
        const days = Math.ceil((depMid - todayMid) / 86400000); 
    ...
    if (todayMid > depMid && todayMid < retMid) { 
        const days = Math.ceil((retMid - todayMid) / 86400000); 
        return `🌴 On vacation (returns in ${days} day${days !== 1 ? 's' : ''})`; 
    } 
    if (todayMid.getTime() === retMid.getTime()) return "📅 Return today – back to work tomorrow"; 
    if (todayMid > retMid) { 
        const days = Math.ceil((todayMid - retMid) / 86400000); 
        return `✅ Back at work (${days} day${days !== 1 ? 's' : ''} ago)`; 
    } 
    return "⏳ Upcoming"; 
}

function getStatusFromDates(departureStr, returnStr, hasPdf) { 
    if (hasPdf) return "Ticket Confirm"; 
    if (!departureStr || !returnStr) return "Draft"; 
    
    const today = getToday();
    const dep = parseDateLocal(departureStr);
    const ret = parseDateLocal(returnStr); 
    
    if (today > ret) return "Completed"; 
    if (today >= dep && today <= ret) return "Ongoing"; 
...
function recalcRowFields(row) { 
    ...
    row.status = getStatusFromDates(row.departure, row.return, row.hasPdf); 
    return row; 
}
...
async function saveToGoogleSheet(dataToSave) { 
    try { 
        const payload = dataToSave.map(e => ({
            employeeName: e.employeeName,
            remarks: e.remarks,
            departure: e.departure,
            return: e.return,
            totalDay: e.vacationDays,
            ticketsTaken: e.ticketsTaken,
            hasPdf: e.hasPdf,
            pdfName: e.pdfName,
...
async function saveModalToGoogleSheet() { 
    if (!isAdmin()) return; 
    if (!sheetData.length) { 
        showModalMessage('No data', 'error'); 
        return; 
    } 
    updateSyncLabel('Saving...'); 
    const success = await saveToGoogleSheet(sheetData); 
    if (success) { 
        ...
        showModalMessage('Error saving', 'error'); 
    } 
}
...
function updateSyncLabel(text, isError = false) { 
    const s = document.getElementById('syncStatusLabel'); 
    if (s) { 
        s.innerText = text; 
        s.style.color = isError ? '#ffb74d' : '#28a745'; 
    } 
}
...
function closeGoogleSheetPopup() { 
    const modal = document.getElementById('googleSheetModal'); 
...
function showMonthlySummary() { 
    document.getElementById('monthlySummaryModal').style.display = 'block'; 
}

function closeSummaryModal() { 
    document.getElementById('monthlySummaryModal').style.display = 'none'; 
}
...
function loadMonthlySummary() {
    ...
    sheetData.forEach(emp => {
        if (emp.departure) { 
            const depDate = parseDateLocal(emp.departure); 
            if (depDate && depDate.getFullYear() === year && depDate.getMonth() === month) departingEmployees.push(emp); 
        }
        if (emp.return) { 
            const retDate = parseDateLocal(emp.return); 
            if (retDate && retDate.getFullYear() === year && retDate.getMonth() === month) returningEmployees.push(emp); 
        }
    });
        ...
        resultDiv.innerHTML = `
            <div class="summary-card" style="background: linear-gradient(135deg, #6c757d, #495057);">
                ...
                <div style="text-align: center; padding: 20px;">
                    ...
                    <p>No employees traveling or returning in this month.</p>
                </div>
            </div>`;
        return;
    }
        ...
        <div class="summary-card">
            ...
            <div class="summary-stats">
                <div class="summary-stat">
                    <div class="summary-stat-number">${departingEmployees.length}</div>
                    <div class="summary-stat-label"><i class="fas fa-plane-departure"></i> Going/Departing</div>
                </div>
                <div class="summary-stat">
                    <div class="summary-stat-number">${returningEmployees.length}</div>
                    <div class="summary-stat-label"><i class="fas fa-plane-arrival"></i> Returning</div>
                </div>
            </div>
        </div>`;
    ...
    if (departingEmployees.length > 0) {
        html += `
            <div class="employee-list">
                <h4><i class="fas fa-plane-departure" style="color: #1e6f5c;"></i> Employees Going/Departing (${departingEmployees.length})</h4>
                <div class="employee-tags">`;
        departingEmployees.forEach(emp => { 
            const depDate = emp.departure ? new Date(emp.departure).toLocaleDateString() : 'Date not set'; 
            html += `<div class="employee-tag">✈️ ${escapeHtml(emp.employeeName)} <span style="font-size: 0.6rem;">(${depDate})</span></div>`; 
        });
        html += `</div></div>`;
    }
    if (returningEmployees.length > 0) {
        html += `
            <div class="employee-list">
                <h4><i class="fas fa-plane-arrival" style="color: #17a2b8;"></i> Employees Returning (${returningEmployees.length})</h4>
                <div class="employee-tags">`;
        returningEmployees.forEach(emp => { 
            const retDate = emp.return ? new Date(emp.return).toLocaleDateString() : 'Date not set'; 
            html += `<div class="employee-tag employee-tag-return">🛬 ${escapeHtml(emp.employeeName)} <span style="font-size: 0.6rem;">(${retDate})</span></div>`; 
        });
        html += `</div></div>`;
    }
...
function addTempRow() {
    ...
    newRow.innerHTML = `
        <td style="font-weight:bold; background:#f8f9fa;">${tbody.children.length + 1}</td>
...
async function saveAllTempRecords() {
    ...
    if (newRecords.length === 0) { 
        alert('Please add at least one employee with name'); 
        return; 
    }
...
function renderModalTable() { 
    ...
    if (!sheetData.length) { 
        tbody.innerHTML = '<tr><td colspan="11" style="text-align:center; padding:50px;">No records. Click "Refresh Data" first.</td></tr>'; 
        return; 
    } 
    ...
    sheetData.forEach((emp, idx) => { 
        ...
        const depFmt = formatDateToYMD(fresh.departure);
        const retFmt = formatDateToYMD(fresh.return); 
        
        if (isEditing) { 
            let pdfSection = `<button class="action-btn upload-pdf-btn" onclick="uploadModalPdf(${idx})">📄 ${fresh.hasPdf ? 'Change PDF' : 'Upload'}</button>`; 
            if (fresh.hasPdf) { 
                pdfSection += `<button class="action-btn delete-pdf-btn" onclick="deleteModalPdf(${idx})" style="background:#dc3545; color:white; margin-left:5px;">❌ Remove</button><div class="pdf-name-display">${fresh.pdfName?.substring(0, 10) || ''}</div>`; 
            } 
            html += `
                <tr style="background:#fff3cd;">
                    <td>${idx + 1}</td>
                    <td><input type="text" id="modal_edit_name_${idx}" value="${escapeHtml(fresh.employeeName)}" style="width:110px"></td>
                    <td><input type="text" id="modal_edit_remarks_${idx}" value="${escapeHtml(fresh.remarks)}" style="width:110px"></td>
...
window.startModalEdit = function(id) { 
    if (isAdmin()) { 
        editingModalRowId = id; 
        renderModalTable(); 
    } 
};
...
window.saveModalEdit = async function(id) { 
    if (!isAdmin()) return; 
    sheetData[id] = {
        ...sheetData[id], 
    ...
    showModalMessage('Record updated!', 'success'); 
};

window.deleteModalRecord = function(id) { 
    if (!isAdmin()) return; 
    if (confirm('Delete permanently?')) { 
        ...
        updateAnalytics(); 
        updateStatsUI(); 
        saveToGoogleSheet(sheetData); 
        showModalMessage('Record deleted', 'success'); 
    } 
};

window.deleteModalPdf = function(id) { 
    if (!isAdmin()) return; 
    if (confirm('Remove PDF?')) { 
        ...
        updateStatsUI(); 
        saveToGoogleSheet(sheetData); 
        showModalMessage('PDF removed', 'success'); 
    } 
};

window.uploadModalPdf = function(id) { 
    if (isAdmin()) { 
        currentModalPdfIndex = id; 
        document.getElementById('modalPdfUploadInput').click(); 
    } 
};

window.viewModalPdfLarge = function(id) { 
    const emp = sheetData[id]; 
    if (emp?.pdfBase64) showLargePdf(emp.pdfBase64, emp.pdfName, emp.employeeName); 
    else alert('No PDF'); 
};

document.getElementById('modalPdfUploadInput').onchange = async function(e) { 
    if (!isAdmin()) return; 
    const file = e.target.files[0]; 
    ...
    reader.onload = async (ev) => { 
        if (currentModalPdfIndex < sheetData.length) { 
            ...
            updateStatsUI(); 
            await saveToGoogleSheet(sheetData); 
            showModalMessage('PDF uploaded!', 'success'); 
        } 
...
function startAutoRefresh() { 
    if (autoRefreshInterval) clearInterval(autoRefreshInterval); 
    autoRefreshInterval = setInterval(() => loadFromGoogleSheet(), 30000); 
}
